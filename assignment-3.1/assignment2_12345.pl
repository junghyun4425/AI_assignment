...
% agent solves task using an agenda based search and tops up energy if it is lower than 40
...
% agent solves task using an agenda based search, keeps track of visited oracles and tops up if energy is below 40
...
% children are added to an agenda
...
% looks for the lowest cost of the list
...
% the agent finishes with the task if the goal is reached
